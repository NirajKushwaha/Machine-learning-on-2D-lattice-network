...
noisePercent = float(input("Enter amount of noise to enter in the data(in Percent): "))

for modelNumber in range(numberOfModels):
    X_train, X_test, y_train, y_test = train_test_split(features, labels, test_size=0.2)
    numberOfNoisyPoints = int((noisePercent/100) * len(y_train))

    for i in range(numberOfNoisyPoints):
        randomNumber = random.randint(0 , len(y_train)-1)
        randomLabel = random.choice([0 , 5 , 9 , 13 , 21 , 25 , 29])

        y_train[randomNumber] = randomLabel
...
plt.title("Neural Network" , fontdict=font)
#plt.xlim(0 , 11)
